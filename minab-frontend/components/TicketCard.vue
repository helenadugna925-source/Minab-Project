...
import {
  ...
  UpdateReservation,
} from "~/utils/constants/queries/reservations";
...
const reserveEvent = () => {
  ...
  const { mutate: reserveEventMutation } = useMutation<ReserveEventResponse>(
    ReserveEventMutation,
    { variables }
  );
  ...
  reserveEventMutation()
    .then(async (result) => {
      if (result?.data?.ReserveEvent.checkoutUrl && props.price !== 0) {
        ...
        const { mutate: updateReservation } = await useMutation(
          UpdateReservation,
          { variables }
        );
        updateReservation()
...
<template>
  ...
  <div
    class="flex flex-row items-start w-64 max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700"
  >
    <div class="p-5">
      <h5
        class="mb-2 text-xl font-bold tracking-tight text-text-default dark:text-white"
      >
        {{ name }}
      </h5>
      ...
      <div v-if="isReservable" class="mt-4">
        <div v-if="isReserved">
          <button
            v-if="isReservedTicket && reservationStatus === 'completed'"
            class="text-white bg-accent/60 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 mr-4 text-center dark:bg-accent/80"
            disabled
          >
            Reserved
          </button>
          <span
            v-else-if="isReservedTicket && reservationStatus === 'pending'"
            class="italic mb-3 font-small text-muted dark:text-gray-400"
            >Reservation pending</span
          >
          >
        </div>
        <div v-else>
          ...
          <button
            v-else
            @click="reserveEvent"
            class="text-white bg-primary hover:bg-accent focus:ring-4 focus:outline-none focus:ring-accent font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 mr-4 text-center dark:bg-primary/90 dark:hover:bg-accent/90 dark:focus:ring-accent"
          >
            Reserve
          </button>
        </div>
    ...
    <button
      v-if="cardType === 'create'"
      type="button"
      class="inline-flex items-center p-2 ms-2 text-sm text-accent/60 bg-transparent rounded-sm hover:bg-accent/20 hover:text-accent dark:hover:bg-accent/800 dark:hover:text-accent-300"
